...
RULES = [
    ...
    Rule(r"\b(qa convergence|qa (?:and )?bug pass|qa loop|test[- ]fix loop|bug[- ]fix loop|severity[- ]gated qa|accepted[- ]minor pass|loop until (?:the )?(?:app|api|ui|tests?|workflow|repository).*(?:clean|stable|no .*bugs?))\b", "aiwf-qa-convergence", "bounded QA test-fix convergence", 39),
